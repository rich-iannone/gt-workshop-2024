...
# Coffee Table: Great Tables

## Start

For this lesson we are going to make a complete table using data in the
`"coffee-sales.json"` file. Though there are lots of datasets available in the
Great Tables package, this is not one of them. We'll import the data using
Polars, resulting in a Polars DataFrame.
...
When making a display table with Great Tables, the first thing is always to
introduce the DataFrame to the `GT()` class. We'll assign this to `gt_tbl` and
