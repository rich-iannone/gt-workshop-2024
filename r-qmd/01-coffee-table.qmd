...
package, this is not one of them. We'll import the CSV data as a tibble object.
...
The first thing to do is to introduce the tibble to the `gt()` function. We'll
...
Great Tables comes with a variety of formatting functions. All of them are of
...
`options` argument
3. Try formatting the `*dollars` columns with `fmt_currency()`, play around with
the options in that function
4. Export the table as an image file. Use `gtsave()` for this task.
